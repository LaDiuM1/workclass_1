/* ---함수[ 함: 상자 / 수 : 숫자]
            ...
            -미리 정의[만들어진]된 수/코드 !!!!! 이유는 아래 3가지 이유
                  -요리{레시피}
                     -라면 스프{소금, 고추가루 , 간장분말 등등}
                     -양념장{}
      1.function = 함수
      ...
      3.사용 목적
            ...
            3. 메모리 *효율성 높일수 있다.
               ...
               -전역번수 : {} 없는곳에서 선언된 메모리는 모든 곳에서 호출 가능
                  *메모리 : 변수/상수/배열/객체 등등
            ...
            4. 함수 선언
                  ...
                  function 함수명(인수1,인수2,인수3){
                     ...
                     *****코드 정의******
                     ...
                     2. 함수명       : 서로 다른 함수간의 식별용 [*js내 동일한 이름불가 / 키워드(let,const,for) 불가]       
                     3. ( )       :인수 [ 함수 안으로 들어가는 코드/변수/배열/객체 등 ]
                                 ,로 구분
                     4.{ }       : '{' 함수 시작 / '}' 함수 끝
                     5.return      :함수 종료/리턴            [*해당 함수를 호출했던 곳으로 반환/리턴]
                                 return 3   ;         함수를 호출했던 곳으로 반환'3' 반환
                                 return ;            반환 값이 없으므로 반환값없이  함수종료
                                 
                     ex)   
                        1. 유재석 이 핸드폰 이 고장나서 서비스센터 간다.
                     ...
                     2. 유재석 이 핸드폰을 폐휴대폰 수거함 넣는다.(반환x)         
                              유재석                     휴대폰 수거함( 함수 )
                              --------고장난핸드폰------->
                                    ( 인수/매개변수 )      함수 실행( 휴대폰 수거함에 등록)
...
function 함수2( x ){}                        //2. 인수( x=임의의 변수/매개변수명 ) /반환 없다.
function 함수3 ( x , y , z){}               //3.인수 다수(, 구분)       /반환없다.
function 함수4( x, y , z){return 3;}         //4.인수 있다. / 반환 값 있다.
function 함수5(){return 5;}                  //5.인수 없다. /반환 값 있다.
function 함수6(){return ;}                  //6.인수 없다 / 반환 값 null =단순 함수종료 역할   
      //* 함수가 종료되는 조건 : 1. '}' 끝났을때 2. return 키워드를 만났을때   [아래 코드 있어도 종료! ]

//2. 함수 호출
console.log('안녕!!')                     //1. 미리 정의된 ( JS 만든 사람) log( 인수 = 출력하고 싶은 문자열)함수
...
함수3( 3, 5, 20);                        //6. 함수호출과 동시에 해당 함수에 데이터 3개 전달[ X =3, Y=5 , Z= 20]
